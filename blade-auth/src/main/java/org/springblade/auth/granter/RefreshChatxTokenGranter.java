...
@Component
@AllArgsConstructor
public class RefreshChatxTokenGranter implements IChatxTokenGranter {
	...
	public Member grant(TokenParameter tokenParameter) {
		...
		if (Func.isNoneBlank(grantType, refreshToken) && grantType.equals(TokenConstant.REFRESH_TOKEN)) {
			...
			if (tokenType.equals(TokenConstant.REFRESH_TOKEN)) {
				R<Member> result = memberClient.memberInfo(Func.toLong(claims.get(TokenConstant.USER_ID)));
